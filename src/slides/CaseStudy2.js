...
/**
 * CaseStudy2: Building a CG + Live Action Design Team
 * 
 ...
 */
const CaseStudy2 = ({ onContentDrop }) => {
  const [hasMedia, setHasMedia] = useState(false);
  const [currentMediaPath, setCurrentMediaPath] = useState(null);

  // Load initial media state from localStorage
  useEffect(() => {
    const savedPath = localStorage.getItem('media-case-study-2');
    if (savedPath) {
      console.log('CaseStudy2 initial load, found path:', savedPath);
      setHasMedia(true);
  ...
  const handleMediaUpdate = (id, filePath, fileType) => {
    console.log('CaseStudy2 received media update:', { id, filePath, fileType });
    if (id === 'case-study-2') {
      if (filePath) {
        setHasMedia(true);
      ...
      <ContentArea>
        ...
        <ProjectDetails>
          <ProjectDetailItem>
            ...
            <ProjectDetailValue>2015-2022</ProjectDetailValue>
          </ProjectDetailItem>
          <ProjectDetailItem>
            <ProjectDetailLabel>Role</ProjectDetailLabel>
            <ProjectDetailValue>Design Team Lead</ProjectDetailValue>
          </ProjectDetailItem>
        </ProjectDetails>
      </ContentArea>
    </SlideContainer>
